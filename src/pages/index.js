import Starter from "./Starter";
import Login from './Auth/Login';
import Home from "./Home";
import AmbilBahan from "./AmbilBahan";
import AntarBahan from "./AntarBahan";
import ListLab from "./Lab/ListLab";
import ListDokter from "./Dokter/ListDokter";
import DetailKegiatan from "./Kegiatan/DetailKegiatan";
import Instansi from "./Instansi";
import PengandataranDokter from "./PengantaranDokter";
import Register from "./Auth/Register";
import Lainnya from "./Lainnya";

export { Starter, Login,Register, Home, ListLab,AmbilBahan,AntarBahan,Instansi,DetailKegiatan,PengandataranDokter,Lainnya,ListDokter }